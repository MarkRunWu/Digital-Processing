...
namespace CSpline
{
	int Cubic_spline::setup_CSpline(vector< OMT::Vector3d > &_points)
	{
		if (_points.size() < 2)
		{
	...
	/*----------------------------------------------------------------------*/
	OMT::Point Cubic_spline::get_position(int _seg, double _t)
	{
		OMT::Point position;
	...
	OMT::Vector3d Cubic_spline::get_tangent(int _seg, double _t)
	{
		OMT::Vector3d tangent_dir;
